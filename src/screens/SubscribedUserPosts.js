...
const SubscribedUserPosts = () => {
    ...
    useEffect(async () => {
        try {
            const res = await axios.get("/api/posts/subpost", {
                headers: {
        ...
        <div className="home-container">
            {data.map(dat => (
                <div key={dat._id} className="card home-card">
                    <h5><Link to={dat?.postedBy?._id === state._id ?`profile`:`/profile/${dat.postedBy._id}`}>{dat?.postedBy?.name} {dat?.postedBy?._id === state._id && (<i className="material-icons" style={{float:'right'}} onClick={() => deletePost(dat._id)}>delete</i>)}</Link></h5>
